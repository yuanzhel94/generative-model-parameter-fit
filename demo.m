...
% %case 2: Voronoi Tessellation search over 5 iterations, 2000 points each iteration, pow
% %are [0.5,1,1.5,2] after iteration 1 to 4 (to determine iteration 2 to 5).
% %group energy for a point is determined using a uniform random value
% %between max and min for the populaton (i.e., rule == 'unif', alternatively, use value 'mean' or 'median').

% strategy = 'vt';
% spec = {repmat(2000,1,5),linspace(0.5,2,4),'unif'};
% n_points = sum(spec{1});
